...
class DB {
     ...
     * 
     */
    public static function find($id, $param = array(), $raw = false) {
        if (count($param) != 0) {
     ...
     */
    public static function findAll($SQL = "", $param = array(), $raw = false) {
        if ($SQL == "") {
     ...
     */
    public static function query($SQL, $param = array(), $raw = false) {
        if ($raw == true) {
            $db = Core::$pdo;
            Core::log(get_called_class() . "#" . __FUNCTION__ . "(): " . $SQL);
            $stmt = $db->prepare($SQL);
            ...
            $SQLArray = db::querySplit($SQL);
            if (static::$SQLautojoin && $raw == false) {
            ...
            if (static::$access == true && static::$SQLrestrict && $raw == false) {
                $access = static::$settings["access"];
                if (is_array($access)) {
                    $gruppe = "G" . Core::$user->Gruppe;
                    if (is_array($access[$gruppe])) {
