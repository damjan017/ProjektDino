...
<?php if ($Zimmertyp->Bild): ?>
<img src="images/<?=htmlspecialchars($Zimmertyp->Bild)?>"
     alt="Zimmer" style="width:100%; max-height:220px; object-fit:cover; border-radius:6px; margin-bottom:10px;" />
<?php endif; ?>
...
<div class="ui-body ui-body-a" style="padding:10px; border-radius:6px; margin-bottom:10px;">
    ...
    <p><strong>Unterkunft:</strong>
        <a href="?task=Unterkunft_detail&id=<?=$Unterkunft->id?>" data-ajax="false">
            <?=htmlspecialchars($Unterkunft->Name)?>
        </a>
    </p>
    <?php endif; ?>
    <p><strong>Betten:</strong> <?=(int)$Zimmertyp->Anzahltbett?> &middot; <?=htmlspecialchars($Zimmertyp->ArtBett)?></p>
    <p><strong>Preis:</strong>
        <?php if ($Zimmertyp->Aktionaktiv && $Zimmertyp->Aktionspreis > 0): ?>
            <s style="color:#999;"><?=number_format($Zimmertyp->Preis, 2, ',', '.')?> €</s>
            <strong style="color:#c00;"><?=number_format($Zimmertyp->Aktionspreis, 2, ',', '.')?> €</strong> / Nacht (Aktion!)
        <?php else: ?>
            <?=number_format($Zimmertyp->Preis, 2, ',', '.')?> € / Nacht
        <?php endif; ?>
    </p>
    <p><strong>Verfügbare Zimmer:</strong> <?=(int)$Zimmertyp->AnzahlVerfuegbarkeit?></p>
</div>
...
<ul data-role="listview" data-inset="true">
    ...
    <li>
        <a href="?task=Buchung_detail&id=<?=$b->id?>" data-ajax="false">
            Buchung #<?=$b->id?> &mdash; <?=htmlspecialchars($b->checkin)?> bis <?=htmlspecialchars($b->checkout)?>
            &mdash; <?=htmlspecialchars($b->Status_literal)?>
        </a>
    </li>
    <?php endforeach; ?>
</ul>
