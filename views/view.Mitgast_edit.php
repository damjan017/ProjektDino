...
?>
<a href="?task=Mitgast_detail&id=<?=$Mitgast->id?>"
   class="ui-btn ui-icon-back ui-btn-icon-notext ui-corner-all"
   style="display:inline-block;" data-ajax="false">Zurück</a>
<h2>Mitgast bearbeiten</h2>
<form id="form_Mitgast" method="post" action="?task=Mitgast_edit&id=<?=$Mitgast->id?>" data-ajax="false">
<div class="ui-field-contain">
    ...
    <select name="_Buchung" id="_Buchung">
        <?php if (is_array($_Buchung)): ?>
            <?php foreach ($_Buchung as $b): ?>
            <option value="<?=$b->id?>" <?=($Mitgast->_Buchung == $b->id ? 'selected' : '')?>>
