...
<div class="ui-body ui-body-a" style="padding:10px; border-radius:6px; margin-bottom:10px;">
    <p><strong>Unterkunftsart:</strong> <?=htmlspecialchars($Unterkunft->Unterkunftsart_literal)?></p>
    <p><strong>Sterne:</strong>
        <?php for ($i = 0; $i < (int)$Unterkunft->Bewertung; $i++): ?>&#9733;<?php endfor; ?>
    </p>
    <p><?=nl2br(htmlspecialchars($Unterkunft->Beschreibung))?></p>
    <?php if ($Adresse): ?>
    <p><strong>Adresse:</strong>
        <?=htmlspecialchars($Adresse->formatiert(null))?>
        <?php if ($Adresse->DistanzzurStadt !== null && $Adresse->DistanzzurStadt !== ""): ?>
        (<?=(int) $Adresse->DistanzzurStadt?> km zum Zentrum)
        <?php endif; ?>
    </p>
    <p><strong>Breitengrad:</strong>
        <?=$Adresse->Breitengrad !== null && $Adresse->Breitengrad !== ""
            ? htmlspecialchars((string) $Adresse->Breitengrad)
            : "Keine Angabe"?>
    </p>
    <p><strong>Längengrad:</strong>
...
<div class="ui-body ui-body-a" style="padding:10px; border-radius:6px; margin-bottom:10px;">
    ...
    <?php if (count($groupedAusstattung) === 0): ?>
        <p>Für diese Unterkunft wurden noch keine Ausstattungsmerkmale hinterlegt.</p>
    <?php else: ?>
        <?php foreach ($groupedAusstattung as $category => $items): ?>
            ...
            <ul style="list-style:none; padding:0; display:flex; flex-wrap:wrap; gap:8px;">
                ...
                <li style="background:#e8f5e9; border-radius:20px; padding:4px 12px; font-size:0.9em;">
...
<?php if (!empty($Zimmertyp_b_list)): ?>
    ...
    <div class="ui-body ui-body-a" style="padding:10px; border-radius:6px; margin-bottom:8px;">
        <?php if ($zim->Bild): ?>
        <img src="images/<?=htmlspecialchars($zim->Bild)?>"
             alt="Zimmer" style="width:100%; max-height:150px; object-fit:cover; border-radius:4px; margin-bottom:6px;" />
        <?php endif; ?>
        <h4 style="margin:0 0 5px;"><?=htmlspecialchars($zim->Bezeichnung_literal)?></h4>
        <p style="margin:0;">
            <?=(int)$zim->Anzahltbett?> Bett(en) &middot; <?=htmlspecialchars($zim->ArtBett)?>
        </p>
        <?php
        $preis = ($zim->Aktionaktiv && $zim->Aktionspreis > 0) ? $zim->Aktionspreis : $zim->Preis;
        ?>
        <p style="color:#007; font-size:1.1em;">
            <?php if ($zim->Aktionaktiv && $zim->Aktionspreis > 0): ?>
                <s style="color:#999;"><?=number_format($zim->Preis, 2, ',', '.')?> €</s>
                <strong style="color:#c00;"><?=number_format($zim->Aktionspreis, 2, ',', '.')?> €</strong> / Nacht
            <?php else: ?>
